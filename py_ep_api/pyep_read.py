...
def time_step_handler(state):
    global one_time , oat_handle,  elec_hvac_handle, solar_handle, zone_cool_handle, zone_heat_handle
    if one_time:
        ...
        zone_cool_handle = api.exchange.get_variable_handle(
            state, u"Zone List Sensible Cooling Rate", u"EntireBuilding")
        zone_heat_handle = api.exchange.get_variable_handle(
            state, u"Zone List Sensible Heating Rate", u"EntireBuilding")
        one_time = False
    ...
    zone_cool_val = api.exchange.get_variable_value(state, zone_cool_handle)
    zone_heat_val = api.exchange.get_variable_value(state, zone_heat_handle)
    oat_lst.append(oat_val)
    ele_hvac_lst.append(elec_hvac_value)
    solar_lst.append(solar_val)
    zone_load_lst.append(zone_heat_val - zone_cool_val)
api = EnergyPlusAPI()
...
api.exchange.request_variable(state, "Zone List Sensible Heating Rate", "EntireBuilding")
...
result["zone_load"] = zone_load_lst
print(len(zone_load_lst))
